...
export default function MyOrderPage() {
    ...
    const [formErros, setFormErros] = useState<z.ZodIssue[]>([]);
    ...
    //  handle form submission data using zod
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();

        // validation form data using zod
        const validation = viewBookingSchema.safeParse(formData);
        if (!validation.success) {
            setFormErros(validation.error.issues);
            return;
        }
        ...
        try {
            ...
            if (response.status === 200 && response.data.data) {
                // navigate to booking details page with booking data if round
                navigate("/my-booking", { state: { bookingDetails: response.data.data, notFounf: false } });
            } else {
                ...
                >
                    <div className="flex w-full flex-col gap-5 rounded-3xl bg-white px-[14px] py-5">
                        <label className="flex flex-col gap-[6px]">
                            ...
                            {formErros.find((error) => error.path.includes("booking_trx_id")) && (
                                <p className="text-sm leading-[21px] text-[#E70011]">
                                    {formErros.find((error) => error.path.includes("booking_trx_id"))?.message}
                                </p>
                            )}
                        </label>
                        <label className="flex flex-col gap-[6px]">
                            ...
                            {formErros.find((error) => error.path.includes("email")) && (
                                <p className="text-sm leading-[21px] text-[#E70011]">
                                    {formErros.find((error) => error.path.includes("email"))?.message}
                                </p>
                            )}
